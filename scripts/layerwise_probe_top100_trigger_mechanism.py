...
def metric_rows(residuals: dict[str, dict[str, dict[str, np.ndarray]]], sample_indices: np.ndarray) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    metrics: list[dict[str, Any]] = []
    samples: list[dict[str, Any]] = []
    for layer in LAYERS:
        clean_adv = residuals["clean0"]["adv"][layer]
        clean_trigger = residuals["clean0"]["trigger"][layer]
        bad_adv = residuals["badnet0"]["adv"][layer]
        bad_trigger = residuals["badnet0"]["trigger"][layer]
        ...
        for position, sample_index in enumerate(sample_indices):
            samples.extend((
                ...
                {"model_alias": "badnet0", "layer": layer, "sample_index": int(sample_index), "adv_alignment": float(bad_alignment[position]), "adv_delta_l2": float(bad_adv_l2[position]), "trigger_delta_l2": float(bad_trigger_l2[position]), "adv_zero_norm": bool(bad_adv_l2[position] <= 1e-12), "trigger_zero_norm": bool(bad_trigger_l2[position] <= 1e-12)},
            ))
...
def main() -> None:
    ...
    if args.train_count > len(train_data) or args.top_k > len(test_data):
    ...
    aliases = ("clean0", "clean1", "clean2", "clean3", "badnet0")
    ...
    config.update({
        "protocol": "stage1d-layerwise-clean123-probe-clean0-top100-clean0-badnet0-v1",
        ...
        "train_pool": "deterministic CIFAR-10 train subset",
    ...
    write_csv(output / "candidate_pool.csv", [{"split": "cifar10_train", "sample_index": index, "pool_role": "probe_train"} for index in train_indices] + [{"split": "cifar10_test", "sample_index": index, "pool_role": "probe_selection"} for index in test_indices])
    ...
    adapter_map = build_trigger_adapters(model_root=backdoorbench_root, backdoorbench_root=backdoorbench_root, explicit={"badnet": Path(args.badnet_trigger_path).expanduser().resolve() if args.badnet_trigger_path else None, "blended": None, "wanet_identity": None, "wanet_noise": None, "ssba_encoder": None, "ssba_config": None, "inputaware": None, "adaptive_blend": None}, device=device, blended_alpha=0.2, wanet_s=0.5, wanet_grid_rescale=1.0)
    adapter = adapter_map["badnet"]
    ...
    for alias_index, alias in enumerate(("clean0", "badnet0")):
        model, _info = load_modelzoo_classifier(alias, model_zoo_root=str(model_zoo_root), device=device)
    ...
    for alias in ("clean0", "badnet0"):
        ...
        del model
    metrics, sample_rows = metric_rows(residuals, np.asarray(selected_indices, dtype=np.int64))
    ...
    np.savez_compressed(output / "endpoint_arrays.npz", sample_indices=np.asarray(selected_indices, dtype=np.int64), original=original_images, clean0_adv=endpoints["clean0"], badnet0_adv=endpoints["badnet0"], badnet_trigger=trigger_images)
    ...
    write_json(output / "summary.json", {"protocol": config["protocol"], "output_directory": str(output), "selected_count": len(selected_indices), "selected_indices": selected_indices, "models": ["clean0", "badnet0"], "trigger_source": adapter.status.source, "metrics": metrics, "model_zoo_provenance": provenance, "attack_success_counts": {alias: sum(row["success"] for row in attack_records if row["model_alias"] == alias) for alias in ("clean0", "badnet0")}, "interpretation_boundary": "Probe-selected Top-100 representation-direction alignment and concentration; not a causal proof."})
